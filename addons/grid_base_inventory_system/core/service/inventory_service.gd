...
## 背包数据库引用
var _inventory_repository: ContainerRepository = ContainerRepository.instance
...
func get_container(inv_name: String) -> ContainerData:
	return _inventory_repository.get_container(inv_name)
	...
	if new_item_data is StackableData:
		...
		for item in items:
			if not item.is_full():
				...
				var new_item_grids = _inventory_repository.get_container(inv_name).find_grids_by_item_data(item)
				...
				if new_item_data.current_amount <= 0:
					return true
	# 增加不可堆叠物品，或堆叠后剩余的物品
	var grids = _inventory_repository.get_container(inv_name).add_item(new_item_data)
...
func find_item_data_by_item_name(inv_name: String, item_name: String) -> Array[ItemData]:
	var inv = _inventory_repository.get_container(inv_name)
	if inv:
...
func find_item_data_by_grid(inv_name: String, grid_id: Vector2i) -> ItemData:
	return _inventory_repository.get_container(inv_name).find_item_data_by_grid(grid_id)
...
func is_inventory_existed(inv_name: String) -> bool:
	return _inventory_repository.get_container(inv_name) != null
...
func quick_move(inv_name: String, grid_id: Vector2i) -> void:
	...
	var item_to_move = _inventory_repository.get_container(inv_name).find_item_data_by_grid(grid_id)
...
func remove_item_by_data(inv_name: String, item_data: ItemData) -> void:
	if _inventory_repository.get_container(inv_name).remove_item(item_data):
